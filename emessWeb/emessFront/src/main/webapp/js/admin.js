...
//加入收藏 2017-9-13 10:10:55
function dr_add_collect(mid, id, catid) {
    $.ajax({
        type: "POST",
        dataType: "JSON",
        url: "/index.php?s=member&c=collect&m=add",
        data: "mid="+mid+"&eid="+id+"&catid="+catid,
        success: function(data) {
            if (data == 2) {
                dr_tips('您还未登录，请先登录！',0);
            } else if(data == 1) {
            	$(this).removeClass('fa-heart-o').addClass('fa-heart')
            	$('.sc i').removeClass('fa-heart-o').addClass('fa-heart')
                $('.sc #scsp').html('已收藏')
                dr_tips('已经加入我的收藏',2,1);
            } else if(data == 3) {
            	$(this).removeClass('fa-heart').addClass('fa-heart-o')
            	$('.sc i').removeClass('fa-heart').addClass('fa-heart-o')
            	$('.sc #scsp').html('收藏商品')
                dr_tips('已经取消收藏',2,1);
            } else {
            	dr_tips('加入收藏失败,请刷新后重试', 0);
            }
        },
        error: function(HttpRequest, ajaxOptions, thrownError) {}
    });
...
$(function() {
    ...
    var gotype = $(".radio-list .radio-inline input[name='data[news_op]']:checked").val();
    $('.xsca').click(function(){
    ...
    if (gotype == 1) {
        $("tr .t2 #list_news_file_files").parents('tr').hide();
        $("#dr_row_news_file").hide();
        $("#dr_row_news_content").hide()
        $("#dr_news_content").parents('tr').hide()
    } else if (gotype == 2) {
        $("tr .t2 #dr_news_link").parents('tr').hide();
        $("#dr_row_news_link").hide();
        $("#dr_row_news_content").hide()
        $("#dr_news_content").parents('tr').hide()
    } else if (gotype == 3) {
        $("tr .t2 #list_news_file_files").parents('tr').hide();
    ...
    $(".radio-list .radio-inline input[name='data[news_op]']").on('change', function() {
        if ($(this).val() == 1) {
            $("tr .t2 #list_news_file_files").parents('tr').hide();
            $("tr .t2 #dr_news_link").parents('tr').fadeIn();
            $("#dr_row_news_file").hide();
            $("#dr_row_news_link").fadeIn();
            $("#dr_row_news_content").hide()
            $("#dr_news_content").parents('tr').hide()
        } else if ($(this).val() == 2) {
            $("tr .t2 #dr_news_link").parents('tr').hide();
            $("tr .t2 #list_news_file_files").parents('tr').fadeIn();
            $("#dr_row_news_link").hide();
            $("#dr_row_news_file").fadeIn();
            $("#dr_row_news_content").hide()
            $("#dr_news_content").parents('tr').hide()
        } else if ($(this).val() == 3) {
            $("tr .t2 #dr_news_link").parents('tr').hide();
            ...
            $("#dr_row_news_link").hide();
            $("#dr_row_news_file").hide();
            $("#dr_news_content").parents('tr').fadeIn()
            $("#dr_row_news_content").fadeIn()
        }
    })
    ...
    $('.ui-sortable .upload-container .pic img').each(function(){
        if($(this).attr('src') == 'javascript:void(0);'){
            $(this).parents('.upload-container').hide()
        }
    });
    $('.ui-sortable').each(function(){
        $(this).find('.upload-container:hidden:first').show()
    })
});
