...
function dr_remove_file(name, id) {
	art.dialog.confirm(lang["confirm"],
		function() {
...
function dr_login() {
	art.dialog.open(memberpath + "index.php?s=member&c=login&m=ajax", {
		title: lang["login"],
		opacity: 0.1,
		lock: true,
		width: 380,
		...
		ok: function() {
			...
			if (value == "" || value == undefined) {
				...
				if (iframe.$("#is_down").attr("checked")) {
					...
					$.ajax({
						...
						success: function(text) {
							...
							if (text.status == 0) {
								...
								$("#show_" + name).html('<a href="javascript:;" onclick="dr_show_file_info(\'' + text.id + '\')"><img align="absmiddle" src=' + text.icon + '><div class="onCorrect">' + text.size + "&nbsp;</div></a>")
							}
						}
					})
				} else {
					...
					$("#dr_" + name).val(info[0]);
					$("#show_" + name).html('<a href="javascript:;" onclick="dr_show_file_info(\'' + info[0] + '\')"><img align="absmiddle" src=' + info[1] + '><div class="onCorrect">' + finfo + "&nbsp;</div></a>");
					return true
				}
			}
		},
